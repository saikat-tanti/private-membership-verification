...
async function main(): Promise<void> {
  ...
  process.stdout.write(`\n→ Setting up private-membership-verification on network: ${network}\n\n`);

  // 1. Bring up only the services this network needs (skip when empty / remote-only).
  if (config.composeServices.length > 0) {
    run('docker', ['compose', 'up', '-d', '--wait', ...config.composeServices]);
  } else {
    process.stdout.write(
      `  No local compose services for ${network} (using remote endpoints).\n\n`,
    );
  }
