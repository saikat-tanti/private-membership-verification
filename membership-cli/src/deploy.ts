/**
 * Deploy private-membership-verification contract to a Midnight network.
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy private-membership-verification to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  const initialGroupName = process.env.INITIAL_GROUP_NAME || 'VIP Founders Club';

  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      deployed = await deployContract(providers, {
        compiledContract: compiledContract as any,
        args: [initialGroupName],
  ...
  console.log(`  Group Name:       ${initialGroupName}`);
